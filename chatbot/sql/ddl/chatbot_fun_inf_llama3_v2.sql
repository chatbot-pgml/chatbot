...
CREATE OR REPLACE FUNCTION chatbot.process_dialogue_v2(p_id_tenacidade integer, p_id_sessao integer, p_id_pessoa integer, p_question text, p_id_arquivo integer DEFAULT 0)
 RETURNS text
 LANGUAGE plpgsql
AS $function$
DECLARE
        ...
        lista_contexto AS (
            ...
            FROM (
                SELECT ap.id_arquivo, ap.parte,
						1 - pgml.cosine_similarity(pgml.embed(me.modelo, p_question)::vector, ap.embed::vector) AS cosine_distance
